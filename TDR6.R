...
sd(slv) # calcul de l'écart-type
min(slv) # afficher la plus petite valeur
max(slv) # afficher la plus grande valeur
range(slv) # afficher la plus petite et la plus grande valeur (amplitude)
sort(slv) # classement des valeurs de la plus petite à la plus grande
rev(slv) # inverser l'ordre des éléments
sum(slv) # calcul de la somme
cumsum(slv) # calcul de sommes (sommer chaque élément)
